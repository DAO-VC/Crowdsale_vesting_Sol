...
async function main() {
  ...
  for (let idx = 1; idx < 24; idx++) {
    ...
    releaseSchedule.push({
      releaseTime: new BN(currentBlockTime + 5 * 60 * idx),
      fraction,
    });
  }
  releaseSchedule.push({
    releaseTime: new BN(currentBlockTime + 5 * 60 * 24),
    fraction: 10000 - sum,
  });

  const pubkeys = await program.methods.initialize(priceNumerator, priceDenominator, paymentMinAmount, 0, releaseSchedule)
    .accounts({
      sale: sale.publicKey,
      authority: AUTHORITY,
      saleMint: SALE_MINT,
  ...
  const tx = await program.methods.initialize(priceNumerator, priceDenominator, paymentMinAmount, 0, releaseSchedule)
    .accounts({
      sale: sale.publicKey,
      authority: AUTHORITY,
      saleMint: SALE_MINT,
  ...
  console.log("Devnet 5 minute private sale initialized");
